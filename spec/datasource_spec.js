...
describe('GenericDatasource', function() {
    ...
    it('should return an error when testing datasource with non 200 response', function(done) {
        ...
        ctx.ds.testDatasource().then(function(result) {
          ...
          expect(result.message).to.equal("Invalid application key");
          done();
        })
    })

    it('should return an success when testing datasource with 200 response', function(done) {
        ctx.backendSrv.datasourceRequest = function(request) {
            return ctx.$q.when({
                _request: request,
                data: {
        ...
        ctx.ds.testDatasource().then(function(result) {
          ...
          expect(result.message).to.equal("Data source is working");
          done();
        });
    })
